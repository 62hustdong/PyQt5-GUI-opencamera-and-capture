...
################################ function support #######################################
def make_save_file(default_config):
    # input: noi dung cac dong can luu
    # Kiem tra file save neu chua co thi tao file save
    file_path = "D:/GUI/save.txt"
    if not os.path.isfile(file_path):
        if not os.path.isdir("D:/GUI"):
            os.makedirs("D:/GUI")
        with open("D:/GUI/save.txt", 'w') as f:
...
class Ui_CameraWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        # Tao danh sach chua ten cac loai camera va danh sach chua kenh
        cam_names = ["Cam 0", "Cam 1"]
        chanel_names = ["RGB", "Gray", "HSV"]
        self.chonCamCB.addItems(cam_names)
        self.chonKenhCB.addItems(chanel_names)
...
class CameraWindow(QMainWindow, Ui_CameraWindow):
    def __init__(self):
        ...
        # Khoi tao cac thuoc tinh cua doi tuong camerawindow
        self.cap = None
        ...
        default_config = ["0",
                          ...
                          "\n1",
                          "\n"]
        # doc file save de load cau hinh, neu mat thong tin thi lay cau hinh mac dinh
        make_save_file(default_config)

        f = open("D:/GUI/save.txt", 'r')
        config = f.readlines()
        print(len(config))
        self.logic = int(config[0])
        self.chanel = int(config[1])
        self.brightness_adjuster = int(config[2])
        self.contrast_adjuster = int(config[3])
        self.waitTime = int(config[4])
        self.soLuong = int(config[5])
        f.close()


        # su kien dong cua so se keo theo xoa man hinh hien thi anh,
        # dong thoi xoa bien cap de giai phong bo nho
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # Khoi tao mot doi tuong thuoc class CameraWindow,
        # qua do cho phep man hinh MainWindow tac dong toi man hinh
        # CameraWindow thong qua doi tuong vua tao
        # Camera Windown
        self.cameraWindow = CameraWindow()
        self.cameraWindow.setupUi(self.cameraWindow)

        # Sau khi load duoc cau hinh thi setup cac phan tu theo cau hinh do
        self.cameraWindow.doubleSpinBox1.setValue(self.cameraWindow.brightness_adjuster)
        self.cameraWindow.doubleSpinBox2.setValue(self.cameraWindow.contrast_adjuster)
        self.cameraWindow.doubleSpinBox3.setValue(self.cameraWindow.waitTime)
        self.cameraWindow.BrightnessSlider.setSliderPosition(self.cameraWindow.brightness_adjuster)
        self.cameraWindow.ContrastSlider.setSliderPosition(self.cameraWindow.contrast_adjuster)
        self.cameraWindow.chonCamCB.setCurrentIndex(self.cameraWindow.logic)
        self.cameraWindow.chonKenhCB.setCurrentIndex(self.cameraWindow.chanel)

        if self.cameraWindow.soLuong == 1:
            self.cameraWindow.x1RB.setChecked(True)

        if self.cameraWindow.soLuong == 10:
            self.cameraWindow.x10RB.setChecked(True)

        if self.cameraWindow.soLuong == 100:
            self.cameraWindow.x100RB.setChecked(True)
        ...
        default_config = ["0",
                          ...
                          "\n1",
                          "\n"]
        # doc file save de load cau hinh, neu mat thong tin thi lay cau hinh mac dinh giong cua CameraWindow
        # chu yeu de lay duong dan
        ...
        self.myRootPath = config[6]
        self.cameraWindow.logic = int(config[0])
        self.cameraWindow.chanel = int(config[1])
        self.cameraWindow.brightness_adjuster = int(config[2])
        self.cameraWindow.contrast_adjuster = int(config[3])
        self.cameraWindow.waitTime = int(config[4])
        self.cameraWindow.soLuong = int(config[5])
        f.close()
    ...
    def doubleSpinBox1_valueChanged(self, value):
        self.cameraWindow.brightness_adjuster = int(value)
        self.cameraWindow.BrightnessSlider.setValue(value)

    def ContrastSlider_valueChanged(self, value):
        self.cameraWindow.contrast_adjuster = int(value)
    ...
    def doubleSpinBox3_valueChanged(self, value):
        self.cameraWindow.waitTime = int(value)
    ...
    def exitBT_clicked(self):
        ...
        reply = QMessageBox.question(self, 'Message',
                                     ...
                                     QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            f = open("D:/GUI/save.txt", "w")

            config = ["{}".format(self.cameraWindow.logic),
                      "\n{}".format(self.cameraWindow.chanel),
                      "\n{}".format(self.cameraWindow.brightness_adjuster),
                      "\n{}".format(self.cameraWindow.contrast_adjuster),
                      "\n{}".format(self.cameraWindow.waitTime),
                      "\n{}".format(self.cameraWindow.soLuong),
                      "\n{}".format(self.myRootPath)
                      ]
            print(config)
